...
// Tüm bu dosya projede halihazýrda kullanýþsýzdýr 1. dönemde yaptýðým projede bir arayüz yoktu ve her iþlem
// konsol üzerinden yapýlmaktaydý projenin geliþimini göz önünde tutmak için bu dosya kasýtlý olarak býrakýlmýþtýr.
public class Kulup {
  ...
  public static void oyuncuCikar(int formaNo) {
    ...
    System.out.println("Oyuncu kulüpten ayrýldý");
  }
  ...
  public static void teknikCýkar(int id) {
    for (int i = 0; i < teknik.size(); i++) {
      if ((int) teknik.get(i).get(0) == id) {
        teknik.remove(i);
      }
    }
    System.out.println("Teknik personel kulüpten ayrýldý");
  }
  ...
  public static void yonetimCikar(String ad_soyad){
    ...
    System.out.println("Yönetim Kurulu üyesi kulüpten ayrýldý");
  }
  ...
  public static String getBaskan(){
    ...
    for (int i = 0; i < yonetim.size(); i++) {
      if ( yonetim.get(i).contains("Baþkan")) {
  ...
  public static void bilgileriGoster(){
    ...
    System.out.println("Kulüp Adý : ProjeSpor"+
    "\nKuruluþ Yýlý : 1883"+
    "\nBaþkan : "+ getBaskan()+
    "\nTeknik Direktör : "+ getDirektor()+
    "\nKadro Deðeri : "+String.format("%.2f",kadro_degeri_hesapla() )+" Milyon £"+
    "\nKadro Yaþ Ortalamasý : "+getYasOrtalamasi()+
    "\nMaaþ Giderleri Toplamý  : "+ String.format("%.2f",maasGiderleriHesapla() )+" Milyon £"+
    "\nKulübün Net Deðeri : "+String.format("%.2f",sonuc )+" Milyon £");
